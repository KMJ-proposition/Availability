...
* \>
	- 파일 내용을 덮어씀
	```
	root# ls /etc > ~/output.txt	# 파일의 내용을 덮어씀 \-\-\> 파일이 없으면 생성됨
	```
* \>\>
	- 파일 내용을 추가함
	- 바이트 단위의 표준 출력 스트림을 파일로 방향을 전환한다.
	```
	root# date >> ~/output.txt	# 내용을 파일에 추가함
	```
* wc
	- word count \-\-\> 단어 갯수
...
* tail
	...
	- 파일의 맨 아랫줄(꼬리) 내용을 출력
	```
	root# tail -n 1 output.txt	# 추가된 내용 확인
		202x. 0x. 0x. (일) 1x:1x:xx KST
	```
...
* 	|
	...
	```
	root# ls /var/log | head -n 4		# -n 옵션으로 줄 수를 지정할 수 있다.
		README
		anaconda
		audit
		boot.log
	```
	```
	root# ls /etc | wc -l > ~/count.txt
		# 순서
		# 1. ls가 /etc 디렉터리 목록을 출력해 스트림을 파이프라인 다음 명령어로 연결한다.
		# 2. 파이프라인 다음 명령어가 그 스트림을 받는다.
		# 3. wc 명령어를 실행하면 줄 수를 출력한다.
		# 4. 리다이렉션으로 내용을 파일로 저장한다.

	root# cat count.txt	# 내용 출력
		272
	```
...
* &>
	...
	```
	root# ls /var/log /invalid/directory &> ~/combined.txt
	root# ls /var/log /invalid/directory > ~/combined.txt 2>&1

	# 바로 위 한 명령줄에 FD 1번, 표준 입력이 이미 '~/combined.txt'로 입력되어 있다.
	# 그러므로 이후에 &1, FD 1번으로 출력하면 이 파일로 리다이렉션 된다.
	```
* 내용 확인
	```
	root# cat combined.txt
		ls: cannot access '/invalid/directory': 그런 파일이나 디렉터리가 없습니다
		/var/log:
		README
		...

	# 오류가 최상단에 위치하였으나, 이상하게 느낀다면 이는 ls가 정한 규칙 때문이다.
	```
...
* 각 파일의 행 갯수 확인
	```
	root# wc out.txt err.txt
		273  273 2595 out.txt
		1    8   89 err.txt
	```
* 내용 확인
	```
	root# tail -n 1 out.txt err.txt
		==> out.txt <==
		yum.repos.d
...
* 표준 출력 + 표준 오류
	```
	root# ls /var/log /invalid/directory >> ~/out.txt 2>> ~/err.txt
	```
* wc로 확인
	```
	root# wc out.txt err.txt
